...
public class KaleidoscopePen {
  ...
  public KaleidoscopePen(PApplet drawTo, int numberOfTrianglePens) {
    ...
    this.pens = new TrianglePen[numberOfTrianglePens];
    for (int i = 0; i < pens.length; i++) {
      pens[i] = new TrianglePen(this.processing, i == 0);
    }
  }

  public void update(int mouseX, int mouseY, boolean mousePressed, char keyPressed) {
    for (int i = 0; i < pens.length; i++) {
      int[] position = rotate(mouseX, mouseY, i * 2 * Math.PI / pens.length);
      pens[i].update(position[0], position[1], mousePressed, keyPressed);
    }
  }
